...
- **⚠ Fusionner déploie Vercel, PAS le worker Railway.** Le worker tourne sur une **image Docker figée**, reconstruite uniquement à la main. Une PR peut donc être fusionnée, la CI verte et Vercel `Ready`, pendant que le worker continue d'exécuter l'ancien code. **Toute PR qui touche du code exécuté par le worker exige un `railway up` après fusion**, et doit le dire dans sa description. Sont concernés : `apps/web/scripts/*-worker*.ts` (closure, veille, reminders, ocr) et **tout ce qu'ils importent** — en particulier `apps/web/src/lib/closure/**` (le `renderer` et **tous les gabarits PDF** : émargement, attestation, certificat…), `packages/db`, `packages/shared`.
  - **`railway up`, JAMAIS `railway redeploy`** : `redeploy` rejoue l'image précédente et ne change donc rien (leçon de la bascule du 03/08/2026).
  - Le job CI `worker-image` ne déploie rien : il prouve seulement que l'image compile (`push: false`). Le voir vert — ou `SKIPPED` — ne dit rien de ce qui tourne en production.
  - Incident de référence, **21/09/2026** : PR #131 (tampon d'émargement en page 2) fusionnée, CI verte, Vercel `Ready` — et le PDF régénéré strictement identique, parce que c'est le worker qui rend l'émargement.
...
- Exceptions historiques préservées :
  ...
  - `/app/diagnostics` (pluriel, FR — chaîne diagnostic lot B, 02/09/2026). Ne pas
