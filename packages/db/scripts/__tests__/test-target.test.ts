...
describe('cibles des tests — refus avant toute écriture', () => {
  it.each([
    undefined,
    'postgresql://x:x@db.gntlqyscahbgjrmsbzil.supabase.co/postgres',
    'postgresql://x:x@remote.example/qualiof_test',
    'postgresql://x:x@localhost/qualiof',
  ])('refuse la cible %s', (databaseUrl) => {
  ...
  it('accepte une base dédiée locale et refuse un navigateur distant, même avec cette base', () => {
    ...
    expect(() =>
      assertTestTarget({ databaseUrl: local, baseUrl: 'https://qualiof.vercel.app' }),
    ).toThrow(/REFUS/);
  });
    ...
    const db = {
      tenant: {
        findMany: vi
          .fn()
          .mockResolvedValue([
            { id: 'db191440-a144-48d1-93c1-767e6f647f2c', name: 'Start Academy' },
          ]),
      },
    };
  ...
  it('refuse une base dont les tenants ne sont pas des fixtures identifiées', async () => {
    const db = {
      tenant: { findMany: vi.fn().mockResolvedValue([{ id: 'real', name: 'Mon organisme' }]) },
    };
