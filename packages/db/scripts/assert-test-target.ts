...
export function assertTestTarget(input: { databaseUrl?: string; baseUrl?: string }): void {
  ...
  if (!target.autorisee || !/_test$/.test(target.base)) {
    throw new Error(
      `REFUS test : base locale dédiée *_test obligatoire (${target.motif ?? 'nom non dédié'}).`,
    );
  }
  const url = new URL(input.databaseUrl!);
  if (!['postgres:', 'postgresql:'].includes(url.protocol))
    throw new Error('REFUS test : protocole PostgreSQL requis.');
  if (input.baseUrl !== undefined) {
    let web: URL;
    try {
      web = new URL(input.baseUrl);
    } catch {
      throw new Error('REFUS test : URL navigateur invalide.');
    }
    if (
      ...
      throw new Error(
        'REFUS test : navigateur réservé à une instance locale dédiée, jamais au déploiement de production.',
      );
    }
  }
}
...
export async function assertTestDatabaseContent(
  ...
  if (
    tenants.some(
      (t) => t.id === 'db191440-a144-48d1-93c1-767e6f647f2c' || /start\s+academy/i.test(t.name),
    )
  ) {
    throw new Error(
      'REFUS test : contenu de production Start Academy détecté. Aucune fixture écrite.',
    );
  }
  if (tenants.some((t) => !/^(TEST|E2E)[-_ ]/i.test(t.name))) {
    throw new Error(
      'REFUS test : tenant non reconnu comme fixture TEST- / E2E-. Utiliser une base jetable vide.',
    );
  }
}
