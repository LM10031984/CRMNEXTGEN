...
import { installFarosLibrary, FAROS_LIBRARY_REF } from '../faros-library';
...
describe('installation des ateliers Faros', () => {
  it('importe une seule fois, conserve les exclusions et refuse d’écraser une relecture', async () => {
    ...
    expect(await db.auditLog.count({ where: { tenantId, action: 'catalogue.faros-applique.installed' } })).toBe(1);
    await db.trainingProduct.update({ where: { id: shelf.id }, data: { excludedFromClientOutputs: true } });
    expect(await installFarosLibrary(db, tenantId)).toBe('unchanged');
    expect((await db.trainingProduct.findUniqueOrThrow({ where: { id: shelf.id } })).excludedFromClientOutputs).toBe(true);
    await db.trainingModule.update({ where: { id: shelf.modules[0]!.id }, data: { contentMd: 'Relecture humaine à conserver' } });
    await expect(installFarosLibrary(db, tenantId)).rejects.toThrow('sans écraser');
    expect((await db.trainingModule.findUniqueOrThrow({ where: { id: shelf.modules[0]!.id } })).contentMd).toBe('Relecture humaine à conserver');
  });
