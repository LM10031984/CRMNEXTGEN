...
const GDPR_EXPORT_TTL_SECONDS = 60 * 60; // 1 heure
 ...
 * `documents/` (tous les PDFs liés) et le stocke dans le bucket docs avec
 * une signed URL TTL 1h.
 *
 * Important : le N° de Sécu est inclus en CLAIR dans le ZIP (c'est le sens
 * même du droit à la portabilité). L'URL signée doit donc être traitée comme
 * un secret par l'admin qui la transmet à l'apprenant.
 */
export async function exportLearnerData(personId: string): Promise<ExportResult> {
  ...
  const signedUrl = await getFileSignedUrl(DOCS_BUCKET, key, {
    expiresIn: GDPR_EXPORT_TTL_SECONDS,
    downloadFilename: `qualiof-rgpd-export-${person.lastName}-${person.firstName}.zip`,
  });
  ...
  return {
    ok: true,
    signedUrl,
    expiresAt: new Date(Date.now() + GDPR_EXPORT_TTL_SECONDS * 1000).toISOString(),
    bytes: zipBuf.length,
  };
}
