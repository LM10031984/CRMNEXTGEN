 ...
 *     individuel, cas SES-0099), le repli sur le siège de l'OF sortait
 *     l'adresse NUE — « 12 avenue des Camélias 06800 Cagnes-sur-Mer » — sans
 *     « Start Academy ». Signalé par Laurent le 11/09/2026 sur des dossiers
 *     déjà signés, avant envoi.
 *
 *  2. **Aucun segment répété.** Le nom d'usage d'un lieu contient souvent déjà
 *     la rue (saisi « Ville — Agence, rue ») ; l'adresse s'affichait alors deux
 *     fois d'affilée.
 *
...
describe('demande AGEFICE — lieu de formation', () => {
  ...
  it("sans lieu rattaché, le repli est SIGNALÉ et non appliqué en silence", async () => {
    ...
    // sinon — rien ne les distingue côté code. La convocation de la même
    // session affiche « à préciser » : sans cet avertissement, les deux
    // documents d'un même dossier se contredisent sans que personne ne le voie.
    expect(r.warnings?.some((w) => /Aucun lieu/.test(w))).toBe(true);
    expect(r.warnings?.some((w) => w.includes('Start Academy'))).toBe(true);
  });

  it('avec un lieu, la raison sociale et le nom du lieu sont composés sans répétition', async () => {
