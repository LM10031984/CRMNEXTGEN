...
vi.mock('@qualiof/db', async (importOriginal) => {
  ...
  const db = {
    ...
    person: {
      findFirst: vi.fn().mockResolvedValue({ id: 'per', firstName: 'Marie', lastName: 'Test' }),
    },
    organization: {
      findFirst: vi
        .fn()
    ...
    sessionParticipant: {
      create: m.participantCreate,
      upsert: m.participantUpsert,
...
vi.mock('@/lib/alertes/formation-notifier', () => ({
  queueSessionCreatedAlert: vi.fn(),
...
vi.mock('@/lib/pricing/company-session-price', () => ({
  synchronizeCompanyPriceTx: vi.fn(),
  assertCompanyPriceEditable: vi.fn(),
}));
vi.mock('@/lib/pricing/declared-session-enrollment', () => ({
  createDeclaredParticipant: m.declaredParticipant,
}));
...
beforeEach(() => {
  ...
  for (const fn of [m.prepare, m.closure, m.conventions, m.convocation, m.agefice])
...
describe('génération exclusivement à la demande', () => {
  ...
  it.each([null, 'INDIVIDUEL', 'ENTREPRISE'])(
    'ajoute un inscrit (%s) sans générer ni régénérer les documents',
    async (regime) => {
      m.sessionFind.mockResolvedValue({
        id: 'ses',
        regime,
        pricePerLearner: 100,
        product: { priceHT: 100 },
