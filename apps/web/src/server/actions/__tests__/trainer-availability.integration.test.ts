...
vi.mock('@qualiof/db', async (original) => {
  const { assertTestTarget } = await import('../../../../../../packages/db/scripts/assert-test-target');
...
import { createTrainerAvailability, updateTrainerAvailability, deleteTrainerAvailability } from '../trainer-availability';
...
let user: User;
const dates = { startsAt: '2026-11-20T08:00:00.000Z', endsAt: '2026-11-20T17:00:00.000Z', status: 'busy' as const, note: 'Indisponible' };
function signIn(actor: User | null) { auth.validateRequest.mockResolvedValue({ user: actor, session: actor ? { id: 'fixture' } : null }); }
async function availability() {
  return db.trainerAvailability.create({ data: { tenantId, trainerId, ...dates, startsAt: new Date(dates.startsAt), endsAt: new Date(dates.endsAt) } });
}
beforeEach(async () => {
  vi.clearAllMocks();
  ...
  user = await db.user.create({ data: { tenantId, email: `${randomUUID()}@planning.invalid`, firstName: 'Test', lastName: 'Admin', hashedPwd: 'unusable-test', role: 'ADMIN' } });
  trainerId = (await db.person.create({ data: { tenantId, firstName: 'Alice', lastName: 'Test', email: user.email } })).id;
  otherTrainerId = (await db.person.create({ data: { tenantId: otherTenantId, firstName: 'Autre', lastName: 'Tenant' } })).id;
  for (const [tid, pid] of [[tenantId, trainerId], [otherTenantId, otherTrainerId]]) await db.externalIdentity.create({ data: { tenantId: tid!, entityType: 'Person.Trainer', entityId: pid!, source: 'test-planning', externalId: randomUUID() } });
  signIn(user);
});
afterEach(async () => {
...
describe('indisponibilités — PostgreSQL réel', () => {
  it('crée, modifie puis supprime avec audit before/after et revalidation', async () => {
    ...
    const id = result.id;
    expect(await db.auditLog.findFirst({ where: { tenantId, entityId: id } })).toMatchObject({ diff: { before: null, after: { trainerId, status: 'busy' } } });
    expect(await updateTrainerAvailability({ id, ...dates, status: 'tentative', note: 'À confirmer' })).toMatchObject({ ok: true, changed: true });
    expect(await db.trainerAvailability.findFirst({ where: { id, tenantId } })).toMatchObject({ status: 'tentative', note: 'À confirmer' });
  ...
  it('refuse startsAt >= endsAt et les statuts hors busy/tentative sans écriture', async () => {
    for (const input of [
      { ...dates, endsAt: dates.startsAt }, { ...dates, startsAt: dates.endsAt, endsAt: dates.startsAt },
      { ...dates, startsAt: 'invalide' }, { ...dates, status: 'available' },
  ...
  it('ne journalise pas une modification sans changement', async () => {
    ...
    expect(await updateTrainerAvailability({ id: a.id, ...dates })).toMatchObject({ ok: true, changed: false });
    expect(await db.auditLog.count({ where: { tenantId } })).toBe(0);
    expect(revalidatePath).not.toHaveBeenCalled();
  });
  it('refuse création, édition et suppression hors tenant même avec ADMIN', async () => {
    const a = await db.trainerAvailability.create({ data: { tenantId: otherTenantId, trainerId: otherTrainerId, startsAt: new Date(dates.startsAt), endsAt: new Date(dates.endsAt), status: 'busy' } });
    expect(await createTrainerAvailability({ trainerId: otherTrainerId, ...dates })).toMatchObject({ ok: false });
  ...
  it('refuse LECTEUR, COMMERCIAL, COMPTABLE et anonyme, y compris pour un ID connu', async () => {
    ...
    for (const role of ['LECTEUR', 'COMMERCIAL', 'COMPTABLE'] as const) {
      ...
      expect(await updateTrainerAvailability({ id: a.id, ...dates, note: 'interdit' })).toMatchObject({ ok: false });
      expect(await deleteTrainerAvailability({ id: a.id })).toMatchObject({ ok: false });
    }
    signIn(null);
    expect(await deleteTrainerAvailability({ id: a.id })).toMatchObject({ ok: false });
  ...
  it('rend busy visible au moteur existant du wizard, tentative ne bloque pas, suppression libère', async () => {
    const result = await createTrainerAvailability({ trainerId, ...dates });
    if (!result.ok) throw new Error(result.error);
    expect(await checkTrainerAvailability(trainerId, [new Date('2026-11-20')], tenantId)).toMatchObject({ hasConflict: true, conflicts: [{ reason: 'unavailable' }] });
    await updateTrainerAvailability({ id: result.id, ...dates, status: 'tentative' });
    expect(await checkTrainerAvailability(trainerId, [new Date('2026-11-20')], tenantId)).toMatchObject({ hasConflict: false });
  ...
  it('FORMATEUR écrit seulement sur sa fiche email unique, sans pouvoir réaffecter une plage', async () => {
    ...
    const sameTenantOther = await db.person.create({ data: { tenantId, firstName: 'Bob', lastName: 'Test' } });
    await db.externalIdentity.create({ data: { tenantId, entityType: 'Person.Trainer', entityId: sameTenantOther.id, source: 'test-planning', externalId: randomUUID() } });
    const others = await db.trainerAvailability.create({ data: { tenantId, trainerId: sameTenantOther.id, startsAt: new Date(dates.startsAt), endsAt: new Date(dates.endsAt), status: 'busy' } });
    ...
    expect(await updateTrainerAvailability({ id: own.id, ...dates, note: 'Moi' })).toMatchObject({ ok: true });
    expect(await createTrainerAvailability({ trainerId: sameTenantOther.id, ...dates })).toMatchObject({ ok: false });
    expect(await updateTrainerAvailability({ id: others.id, ...dates, note: 'Autre' })).toMatchObject({ ok: false });
    expect(await deleteTrainerAvailability({ id: others.id })).toMatchObject({ ok: false });
    expect(await updateTrainerAvailability({ id: own.id, ...dates, trainerId: sameTenantOther.id })).toMatchObject({ ok: false });
  ...
  it('FORMATEUR reste en lecture si email absent, ambigu, fiche archivée ou non formateur', async () => {
    ...
    const duplicate = await db.person.create({ data: { tenantId, firstName: 'Double', lastName: 'Test', email: user.email } });
    expect(await createTrainerAvailability({ trainerId, ...dates })).toMatchObject({ ok: false });
