...
/**
 * Server actions Paramètres OF (Phase 7 Plan 07-02 Task 3, gardes Phase 8 Plan 08-06).
 *
 * 4 actions discriminées `{ ok, ... }` qui :
 *  1. Vérifient le rôle via `requireRole(['ADMIN'])` (D-02 : Paramètres OF =
 *     ADMIN-only). Phase 7 utilisait `validateRequest()` ; Plan 08-06 a serré
 *     en ADMIN strict pour empêcher tout autre rôle d'éditer les paramètres
 *     organisme.
