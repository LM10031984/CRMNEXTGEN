...
const mocks = vi.hoisted(() => ({
  tx: {
    legalLink: { findMany: vi.fn(), create: vi.fn() },
    trainingSession: { findFirst: vi.fn() },
    sessionParticipant: { findUnique: vi.fn(), create: vi.fn() },
...
vi.mock('@qualiof/db', async (original) => ({
  ...(await original<any>()),
  prisma: { $transaction: async (fn: any) => fn(mocks.tx) },
}));
vi.mock('@/lib/sessions/enrollment-regime-guard', () => ({ assertSessionPayersTx: vi.fn() }));
vi.mock('../company-session-price', () => ({
  assertCompanyPriceEditable: mocks.editable,
  synchronizeCompanyPriceTx: mocks.sync,
}));
import { createDeclaredParticipant } from '../declared-session-enrollment';
beforeEach(() => {
  vi.clearAllMocks();
  mocks.tx.trainingSession.findFirst.mockResolvedValue({
    id: 's',
    tenantId: 't',
    regime: 'ENTREPRISE',
    priceTotalHT: 240,
  ...
  mocks.tx.legalLink.findMany.mockResolvedValue([
...
it('ajoute et répartit dans la même transaction sans accepter un tarif individuel', async () => {
  ...
  expect(mocks.editable).toHaveBeenCalledWith(
    mocks.tx,
    expect.objectContaining({ priceTotalHT: 240 }),
  );
  expect(mocks.tx.sessionParticipant.create).toHaveBeenCalledWith({
    data: expect.objectContaining({ priceHT: expect.anything(), sessionId: 's' }),
  });
...
it('le rejeu ne réinscrit ni ne change silencieusement le commanditaire', async () => {
  ...
  expect(await createDeclaredParticipant({ tenantId: 't', id: 'u' }, input)).toEqual({
    id: 'p',
    sponsorOrgId: 'org',
  });
  expect(mocks.tx.sessionParticipant.create).not.toHaveBeenCalled();
  await expect(
    createDeclaredParticipant({ tenantId: 't', id: 'u' }, { ...input, sponsorOrgId: 'autre' }),
...
it('ne crée rien si le forfait est déjà engagé', async () => {
  ...
  await expect(createDeclaredParticipant({ tenantId: 't', id: 'u' }, input)).rejects.toThrow(
    /engagée/,
  );
  expect(mocks.tx.sessionParticipant.create).not.toHaveBeenCalled();
});
