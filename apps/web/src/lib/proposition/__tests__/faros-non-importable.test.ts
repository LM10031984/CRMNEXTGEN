 ...
 *
 * ## CETTE PR NE SE FUSIONNE PAS. ELLE *EST* LA DÉCISION.
 *
 * Ce fichier ne reviendra dans `main` que le jour où ses trois conditions seront
 * remplies — et il sera vert ce jour-là. Le fusionner avant rendrait la CI rouge
 * en permanence, ce qui revient à débrancher la gate pour tout le dépôt
 * (§4 quaterdecies). Le supprimer effacerait un arbitrage de Laurent.
 *
 * Une PR ouverte est la seule forme qui tienne les deux : la décision reste
 * lisible, datée, et elle n'empêche personne de livrer.
 *
 ...
 *
 * ## Ce qui est DÉJÀ livré, et qui n'est pas ici
 *
 * L'exclusion Faros est **sur le chemin réel** depuis le 17/09/2026 :
 * `sortDuRayon` (`packages/db/scripts/lib/barriere-faros.ts`) refuse tout rayon
 * `faros:` avant toute écriture, et `barriere-faros-chemin-reel.test.ts` le
 * vérifie en VERT, dans `main`. Aucun contenu Faros ne peut entrer au catalogue.
 *
 * Ce fichier-ci ne protège donc plus rien : il **tient les conditions de levée**,
 * et rien d'autre. C'est pour ça qu'il peut rester dehors sans risque.
 *
