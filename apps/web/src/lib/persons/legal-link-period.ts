...
export type SessionPeriod = { startDate: Date | string; endDate: Date | string };
...
export function legalLinkAtSession<T extends PeriodLink>(
  links: readonly T[], organizationId: string, session: SessionPeriod | undefined,
): T | null {
  if (!session?.startDate || !session.endDate) {
    // Compatibilité des anciens appelants sans contexte ; ne pas choisir entre périodes datées.
    if (links.some((l) => l.organizationId === organizationId && (l.startDate || l.endDate))) throw new Error('Dates de session requises pour résoudre ce rattachement.');
  ...
  const overlaps = links.filter((l) => l.organizationId === organizationId &&
    (!l.startDate || calendarDay(l.startDate) <= end) &&
    (!l.endDate || calendarDay(l.endDate) >= start));
  if (overlaps.some((l) => (l.startDate && calendarDay(l.startDate) > start) ||
      (l.endDate && calendarDay(l.endDate) < end))) {
    throw new Error('Le rattachement change pendant la session. Corrigez ses dates dans la fiche apprenant ou scindez la session.');
  }
  if (overlaps.length > 1) {
    throw new Error('Plusieurs rattachements sont actifs dans cette organisation aux dates de la session. Corrigez leurs périodes dans la fiche apprenant.');
  }
  return overlaps[0] ?? null;
}

export function planLegalLinkChange(
  link: PeriodLink, input: { role: string; effectiveDate: string },
  ...
  if (link.endDate && start > calendarDay(link.endDate)) {
    throw new Error('Ce rattachement est déjà terminé à cette date. Créez une nouvelle période depuis la fiche apprenant.');
  }
  ...
  return {
    ...
    next: { organizationId: link.organizationId, role: input.role, function: link.function ?? null,
      startDate: start, endDate: link.endDate ? calendarDay(link.endDate) : null, isPrimary: link.isPrimary ?? false },
  };
}

/** Toutes les casquettes actives, une par organisation, sans préférence juridique. */
export function activeLegalLinksAtSession<T extends PeriodLink>(links: readonly T[], session: SessionPeriod | undefined): T[] {
  return [...new Set(links.map((l) => l.organizationId))]
    .map((id) => legalLinkAtSession(links, id, session)).filter((l): l is T => l !== null);
}
