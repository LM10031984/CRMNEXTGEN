...
const link = { id: 'old', organizationId: 'agency', role: 'AGENT_COMMERCIAL', function: null, startDate: null, endDate: null, isPrimary: true };
...
describe('rattachement à la date de la session', () => {
  ...
  it('conserve le rôle de janvier malgré un emploi ultérieur et une EI toujours active', () => {
    expect(legalLinkAtSession(periods, 'agency', session('2026-01-20'))?.role).toBe('AGENT_COMMERCIAL');
  ...
  it('refuse une transition pendant la session et des périodes ambiguës', () => {
    expect(() => legalLinkAtSession(periods, 'agency', session('2026-01-30', '2026-02-02'))).toThrow(/pendant/);
    expect(() => legalLinkAtSession([link, { ...link, id: 'other', role: 'SALARIE' }], 'agency', session('2026-11-20'))).toThrow(/Plusieurs/);
  });
});
describe('préparer un changement de rôle sans écraser l’ancien', () => {
  ...
  it('refuse une date invalide ou précédant le début du lien', () => {
    expect(() => planLegalLinkChange(link, { role: 'SALARIE', effectiveDate: '2026-02-30' })).toThrow(/date/i);
    expect(() => planLegalLinkChange({ ...link, startDate: '2026-03-01' }, { role: 'SALARIE', effectiveDate: '2026-02-01' })).toThrow(/début/);
  });
});
