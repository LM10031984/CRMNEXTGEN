...
export type PlanningSession = {
  ...
  code: string;
  name: string | null;
  regime: 'INDIVIDUEL' | 'ENTREPRISE' | null;
  status: string;
  startDate: string;
  endDate: string;
  productName: string;
...
export function daysInRange(range: PlanningRange): string[] {
  const days: string[] = [];
  for (let day = dayKey(range.start); day <= dayKey(range.end); day = shiftDay(day, 1))
    days.push(day);
  return days;
}
export function planningStatuses(filters: PlanningFilters = {}): string[] {
  const statuses = (filters.statuses ?? DEFAULT_STATUSES).filter(
    (s) => s !== 'CANCELLED' && s !== 'DRAFT',
  );
...
export function buildPlanningGrid(
  ...
  const rows = trainers
    .filter((t) => !filters.trainerIds?.length || filters.trainerIds.includes(t.id))
    .map((trainer) => {
      const assigned = visibleSessions.filter((s) =>
        s.trainers.some((t) => t.personId === trainer.id),
      );
      const absences = availabilities.filter(
        (a) => a.trainerId === trainer.id && ['busy', 'tentative'].includes(a.status),
      );
      ...
      const cells: PlanningCell[] = days.map((date) => {
        ...
        for (const session of assigned) {
          ...
          if (
            estimated
              ? date < dayKey(session.startDate) ||
                date > dayKey(session.endDate) ||
                weekday === 0 ||
                weekday === 6
              : slots.length === 0
          )
        ...
        if (cellSessions.length && cellAbsences.some((a) => a.status === 'busy')) {
          ...
          conflicts.push({
            ...
            date,
            type: 'unavailable',
            sessionIds: cellSessions.map((s) => s.id),
          });
        }
