/**
 * Les 4 journées « L'Agent Incomparable » du diagnostic du stand — LE CONTENU.
 *
 * Module NEUTRE (zéro prisma / auth / React), délibérément séparé du script qui
 * l'écrit en base. La CI l'a imposé le 02/09/2026 : un test qui vérifiait ce
 * contenu en interrogeant la base passait en local (base cloud, catalogue
 * présent) et échouait en intégration continue (base éphémère, catalogue vide).
 * Le contenu des journées est du CODE, il se vérifie sans base ; ce qui exige
 * une base, c'est l'état du déploiement, et c'est une autre question.
 *
 * POURQUOI CES 4 JOURNÉES
 *
 ...
 *
 * Elles sont assemblées à partir du contenu Faros
 ...
 * sont conservées en second membre de chaque ligne — sous forme de donnée
 ...
 */
export const PRIX_JOURNEE_HT = '336.00';
 ...
 */
export const CODES_FAROS: ReadonlySet<string> = new Set(['FRM-0004', 'FRM-0005', 'FRM-0006', 'FRM-0007']);

/** Une ligne du déroulé, et la capsule Faros dont elle vient. */
export type Ligne = readonly [texte: string, capsule: string];
...
export function composerProgramme(blocs: readonly Bloc[]): string {
  return blocs
...
export function capsules(blocs: readonly Bloc[]): string[] {
...
export const JOURNEES_FAROS: readonly Journee[] = [
  {
    code: 'FRM-0004',
