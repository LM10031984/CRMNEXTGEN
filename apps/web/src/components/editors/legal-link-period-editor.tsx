...
export function LegalLinkPeriodEditor({ link }: Props) {
  ...
  const [role, setRole] = useState<NonNullable<EditInput['changeRole']>['role']>(
    link.role === 'SALARIE' ? 'DIRIGEANT' : 'SALARIE',
  );
  ...
  function submit(apply: boolean) {
    ...
    startTransition(async () => {
      const result = await updateLegalLink({
        ...input,
        apply,
        confirmationKey: apply ? review?.result.confirmationKey : undefined,
      });
      if (!result.ok) {
  ...
  return (
    <div className="mt-3 space-y-3 rounded-md border border-border p-3">
      ...
      <p className="text-xs text-muted-foreground">
        Un changement de rôle termine l’ancienne période et en ouvre une nouvelle dans la même
        organisation.
      </p>
      <fieldset disabled={pending || !!review} className="space-y-3 disabled:opacity-60">
        <label className="block text-xs">
          ...
          <select className={fieldClass} value={mode} onChange={(e) => setMode(e.target.value)}>
            <option value="dates">Modifier les dates ou terminer le rattachement</option>
            <option value="role">Changer de rôle</option>
          </select>
        </label>
        {mode === 'role' ? (
          <>
          ...
          <>
            ...
            <label className="block text-xs">
              Fonction
              <input className={fieldClass} value={job} onChange={(e) => setJob(e.target.value)} />
            </label>
            <p className="text-xs text-muted-foreground">
              Une date vide laisse cette borne ouverte.
            </p>
          </>
        )}
      </fieldset>
      {review?.result.preview && (
        <div className="rounded bg-muted p-3 text-sm" role="status">
          <p>
            {label(review.result.preview.after.role)} :{' '}
            {review.result.preview.after.startDate ?? 'début non renseigné'} →{' '}
            {review.result.preview.after.endDate ?? 'sans fin'}
          </p>
          {review.result.preview.next && (
            <p>
              {label(review.result.preview.next.role)} : à partir du{' '}
              {review.result.preview.next.startDate}
            </p>
          )}
          <p className="mt-1 text-xs">
            L’historique sera conservé. Aucun document ni dossier existant ne sera réécrit.
          </p>
        </div>
      )}
      {error && (
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {review && (
          <button type="button" className="text-sm" onClick={() => setReview(null)}>
            Revenir aux dates
          </button>
        )}
        <button
