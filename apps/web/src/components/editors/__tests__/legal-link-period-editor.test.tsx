...
it('montre la période puis exige la revue avant la confirmation', async () => {
  update.mockResolvedValueOnce({ ok: true, changed: false, confirmationKey: 'reviewed', preview: {
  ...
  render(<LegalLinkPeriodEditor link={{ id: 'link', role: 'AGENT_COMMERCIAL', startDate: null, endDate: null, function: null }} />);
  ...
  await waitFor(() => expect(update).toHaveBeenLastCalledWith(expect.objectContaining({ apply: true, confirmationKey: 'reviewed' })));
});
