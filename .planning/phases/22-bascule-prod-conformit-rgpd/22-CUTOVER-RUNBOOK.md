...
| Flip `MAIL_DRY_RUN=false` ×2 | capture Vercel (+ redeploy) ET Railway (redeploy auto) | ✅ **FAIT le 2026-08-04** (après livraison 22-11 fail-closed + credentials Workspace valides — nodemailer verify() AUTH OK smtp.gmail.com:587). Railway : `MAIL_DRY_RUN=false` + `MAIL_FROM` posés via CLI --set, vérifiés post-pose, redeploy auto SUCCESS (bf399f07). Vercel : pose stdin CLI a produit une **valeur vide 2×** (classe PROD-0674, y compris avec newline) → posée par **API REST JSON** (POST 201), vérifiée par pull `MAIL_DRY_RUN="false"`, redeploy alias Ready. ⚠ Leçon durcie : sur Vercel, poser TOUTE variable par API JSON, le CLI stdin est interdit. | 2026-08-04 |
| Email test réel | `messageId` SMTP vers laurent@start-academy.fr (pas `dryRun`) | ✅ **PROUVÉ le 2026-08-04** : 1ʳᵉ activation réelle par Laurent dans Paramètres → « Envois d'emails » (interrupteur général OFF, catégorie « notifications internes » cochée, session test SES-0094) puis script one-shot `sendMail` avec `context: { tenantId, category: 'internal_notification', sessionId: SES-0094 }` → `{ ok: true, messageId: <6ce098d6-0069-405b-5394-9c5e59998164@start-academy.fr> }` SANS dryRun, expéditeur `QualiOF <formation@start-academy.fr>`, **réception confirmée par Laurent** (« ok good ! »). Incident annexe résolu au passage : crash RSC de la page Paramètres (digest 4036615754, pattern render-prop Phase 7 jamais rendu en prod serverless) → fix PR #10 mergée (merge commit b3f0d182), 1208/1208 tests, déployée. | 2026-08-04 |

### 9.5 Invitations équipe (§5) — plan 22-09 (détail complet : `22-TEAM-INVITES.md`)

| Preuve | Attendu | Résultat | Date |
| --- | --- | --- | --- |
| Liste des invités | nom + rôle RBAC par personne (sans PII inutile) | ✅ 3 membres, liste fournie par Laurent (checkpoint Task 1) : **Béatrice L. — ADMIN**, **Jean-Guy O. — ADMIN**, **Laurent M. — ADMIN** (aucun compte existant sous son email officiel — vérifié, pas de doublon ; 2 comptes legacy seed `@startacademy.fr` sans tiret consignés, hors périmètre) | 2026-08-04 |
| Envoi des invitations | emails d'invitation réellement partis (flux `inviteUser`) | ✅ **3/3 partis réellement** (messageId SMTP, `dryRun=false`, `suppressed=false`) via `_invite-team.ts` répliquant le flux `inviteUser` à l'identique (transaction User+UserInvitation, template Phase 8, catégorie `user_invitation`, AuditLog `users.invite`). Garde-fou 22-11 vérifié OUVERT en base AVANT envoi (`emailsEnabled=true` 13:09:10Z + `userInvitationsEnabled=true`) — 4 relevés antérieurs garde fermé = 0 envoi. Users créés ADMIN×3 tenant OK (contrôle Prisma collé au rapport). Liens vers `https://qualiof.vercel.app/invitation/<token>`, expiration 2026-08-11. **12/08 : liens expirés sans clic Béatrice/Jean-Guy → RENVOI à Béatrice seule** (flux `resendInvitation` répliqué, garde 22-11 re-vérifié ouvert, messageId `<b6b12660…>`, nouveau lien expire **19/08**). **Laurent a accepté SON invitation et s'est connecté le 04/08 13:33Z** (lastLoginAt + AuthSession — flux invitation→connexion prouvé de bout en bout, mais pas « tierce »). Première connexion tierce : _(checkpoint en cours — preuve `lastLoginAt`/`AuthSession` Béatrice ou Jean-Guy à coller au rapport)_ | 2026-08-12 |
