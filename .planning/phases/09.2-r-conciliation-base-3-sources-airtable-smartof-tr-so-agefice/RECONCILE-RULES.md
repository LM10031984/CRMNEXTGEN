...
- **Catalogue produits (triage `.planning/audit/TRIAGE-PRODUITS.csv`, prérequis T2/T3)** : 17 RÉEL (sains) · 5 DOUBLON (paires Cadastre stub PROD-0664/0666 à fusionner/supprimer + PROD-0662 sur-mergé déjà ici) · 8 ORPHELIN (catalogue mort sans session, dont stubs des placeholders supprimés) · 1 PRÉINSCRIPTION (FRM-0002) · 1 À_VÉRIFIER (PROD-0043). Nettoyage catalogue = prérequis avant T2/T3, à traiter au Plan 07 / post-audit. **Verdict produit s'écrit ICI (reliquat), pas en double.**
  - **⚠️ DESTRUCTIF + TERRITOIRE NON OUTILLÉ (à épingler avant le Plan 07)** : fusionner/purger des produits est destructif ET hors couverture testée. `dedupe.ts` + `dedupe.merge.test.ts` (Wave 0) ne couvrent QUE `Organization` et `Person` — **or les produits portent aussi des `ExternalIdentity`** (PROD-0662 = 3 UID smartof en sont la preuve). Une fusion/purge produit **sans repointage des EI recréerait les doublons au premier rejeu SmartOF** — exactement le bug que le test Wave 0 verrouille pour les personnes. **Protocole obligatoire au Plan 07** : (1) liste → (2) mot de Laurent → (3) pg_dump → (4) action → (5) **repointage/purge des ExternalIdentity produit** → (6) check SmartOF amont (le produit existe-t-il encore côté SmartOF ? sinon renaissance au rejeu). **Mécanisme** : soit **étendre dedupe au grain produit** (avec un test de fusion produit équivalent au §3.2), soit **SQL manuel revu ligne à ligne** — **JAMAIS un script improvisé à chaud.**
