...
| **C.3** | 🟨 **code livré le 11/09/2026, preuve d'acceptation à faire** | Route `POST /api/webhooks/docuseal` (secret HMAC fail-closed, octets exacts, idempotence `(providerId, eventType, signerKey)` — la clé porte le SIGNATAIRE, `form.completed` partant une fois par signataire). `server/signature-retour.ts` traite les quatre événements ; `lib/signature/retour.ts` porte les décisions pures. `form.completed` ⇒ `PARTIALLY_SIGNED` + `signedAt` + email « à votre tour » au suivant — **jamais `DONE`**, qui signifie « la preuve est en bucket ». `submission.completed` ⇒ PDF signé + certificat téléchargés PUIS écrits en une transaction (`signedPdfUrl` / `signedAt` / `signatureKind=E_SIGNATURE` / `status=signed`), notification ADMIN, exemplaire envoyé aux seuls signataires avec les deux fichiers. Refus et expiration relâchent la pièce par le chemin PARTAGÉ avec l'annulation (`server/signature-relacher.ts`). Crons `signature-reminders` (J+3/J+7, compteur consommé sur départ réel seulement) et `signature-sync` (filet des webhooks perdus, > 1 h, même clé d'idempotence que le webhook). **Les six routes cron sont déclarées dans `vercel.json`** — les trois orphelines comprises. **Recette jouée sur l'aperçu Vercel le 11/09/2026** (`.planning/specs/evidence/signature-C3/JOURNAL-RECETTE.md`, session témoin DEMO-SIG-01) : envoi réel, email « Convention à signer », signature client, `form.completed` → `PARTIALLY_SIGNED`, email « à votre tour », signature OF, `submission.completed`, cellule verte sans rien faire, exemplaire signé reçu avec ses 2 PJ, et Adobe Reader déclarant « signé au moyen de signatures valables » — source de confiance **AATL**, 2 signatures visibles (`11-adobe-panneau-signatures.png`). **Le moteur est prouvé de bout en bout.** Deux défauts d'ÉCRAN, corrigés le 11/09/2026 (ci-dessous). ⚠ **Reste à rejouer** sur l'aperçu les seules étapes que ces défauts touchaient — 4, 6 et 10 — puis les réglages de sortie d'aperçu listés au journal. |
| **C.3 écran** | ✅ **corrigé le 11/09/2026** | **D-C3-1** — le bloc « Signature » affichait « En attente de signature » même après la signature du client : pas de date, aucune distinction entre qui a signé et qui est attendu, et **aucun lien « Signer maintenant »** pour l'organisme, dont c'était pourtant le tour. Le calcul existait et était testé (`ordreSignatairesEnvoyes`) — il ne servait qu'au récapitulatif. `SignatureRequest.signers` remonte désormais jusqu'à la fiche session (jointure sur le `findMany` déjà présent, relecture par `parseSignatureSigners`, rangement par camp par `signatairesDeLaDemande` — **extrait** de `signature-retour.ts` vers `envoi-contrats.ts`, jamais recopié). Une pièce PARTIE lit l'ordre RÉEL ; chaque rang porte « a signé le 11/09/2026 à 18:01 » (fuseau `Europe/Paris` fixé) ; le lien de l'organisme existe quand son tour est venu, et quand il n'existe pas la phrase dit pourquoi. **Trois phrases devenues fausses** sont réécrites : `PHRASE_AUCUN_EMAIL` (supprimée au profit de `mentionAttentePiece`), `mentionAttenteOf` et `MENTION_RETOUR_AUTOMATIQUE` ne promettent plus un branchement livré. **D-C3-2** — `getNotifications()` filtrait `type: 'lead.assigned'` en dur : les lignes `signature.completed` écrites par `prevenirAdmins` depuis C.3 n'étaient lues par personne. Le filtre porte les DEUX types, le payload est relu par `SignatureCompletedPayloadSchema`, libellé et destination viennent d'un module pur (`lib/signature/notification-cloche.ts` — l'attestation d'assiduité ouvre l'onglet « Après », les panneaux inactifs étant rendus `hidden`). `markNotificationRead` réutilisé tel quel. **Dix mutations exécutées et rouges**, dont la substitution `signataires: []` que `tsc` ne voit pas. |
| **D** | ✅ **livré le 12/09/2026** (branche `feat/signature-lot-d`) | **D-C3-5** — `lib/signature/certificat-signature.ts` (nom parlant) + `GET /api/signature-requests/[id]/audit-trail` + lien « Certificat de signature » à CÔTÉ d'« Ouvrir » sur une ligne signée ; `certificatDeLaPiece` n'offre le lien que s'il mène quelque part (un scan du lot A n'a aucun certificat). **Dossier de financement** : `versionAJoindre` prend `signedPdfUrl` (règle n°2), les certificats partent en pièces jointes (règle n°3, UN par demande), le destinataire d'un dossier AGEFICE est le POINT D'ACCUEIL et non le commanditaire — sans repli silencieux —, refus nominatif « dossier incomplet » avec `force` réservé à ADMIN, expéditeur en copie (`cc`). **Écran « Dossier prêt »** : bannière d'état, mention « Signée / Non signée » sur les seules pièces exigées, « Envoyer quand même » qui existe ou n'existe pas. **Pack audit** : sous-dossier `signes/` (PDF signés + certificats), à CÔTÉ des originaux. **Alerte J-15** : `Task` + notification ADMIN/MANAGER, idempotente par le titre de la tâche, muette sur une convention signée à la main et sur une session sans inscrit. **D-2** : le filtre `signed` de la liste porte enfin sur `Document`, et sa puce existe. **D-C3-4** : le bloc muet nomme les inscrits pour qui il n'y a rien à signer. **D-C3-3** : la fiche d'une EI suit la cascade EI_SELF au lieu de celle des agences. ⚠ Un défaut trouvé et corrigé en cours de lot : deux certificats d'un même dossier portaient le même nom de fichier. |
